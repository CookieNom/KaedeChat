...
def stamp(root: Path, tag: str) -> None:
    ...
    for name, pattern in (
        ...
        text, count = re.subn(pattern, lambda m: m[1] + version + m[2], path.read_text(encoding="utf-8"))
        if count != 1:
...
def check(root: Path, tag: str | None = None, rust_version: str | None = None) -> None:
    ...
    config = json.loads((desktop / "tauri/src-tauri/tauri.conf.json").read_text(encoding="utf-8"))
    cargo = tomllib.loads((desktop / "tauri/src-tauri/Cargo.toml").read_text(encoding="utf-8"))
    lock = tomllib.loads((desktop / "Cargo.lock").read_text(encoding="utf-8"))
    ...
    if f"PODFILE CHECKSUM: {checksum}" not in (ios / "Podfile.lock").read_text(encoding="utf-8").splitlines():
