...
Vendored from crates.io Tao 0.35.3 for Tauri's `tauri-runtime-wry` dependency.
Backports [upstream fix #1264](https://github.com/tauri-apps/tao/pull/1264):
release the window-state lock before updating taskbar visibility in the
`TaskbarCreated` handler. Windows can synchronously reenter the window procedure
during the taskbar COM call; holding the lock deadlocks the UI thread.

Remove this patch when Tauri's runtime uses a Tao release containing that fix.
The separate Tao 0.36 dependency already contains it.

On Windows, verify that Kaede stays responsive after restarting Explorer and
disabling/re-enabling a monitor. The original hang is timing-dependent, so repeat
these actions; a single successful attempt does not establish that it is fixed.
