...
pub mod wmf {
    ...
    use std::ffi::c_void;
    use std::{
        borrow::Cow, cell::Cell, marker::PhantomData, mem::MaybeUninit, rc::Rc,
    ...
    // Each scan and capture owns a reference on its calling thread. COM objects
    // must be released before this guard, including on constructor failures.
    ...
    impl MediaFoundationRuntime {
        fn new() -> Result<Self, NokhwaError> {
            let result = unsafe { CoInitializeEx(None, COINIT_MULTITHREADED) };
            let owns_com = result.is_ok();
            // Reuse an existing STA without balancing a COM call that failed.
            if !owns_com && result != RPC_E_CHANGED_MODE {
                return Err(NokhwaError::InitializeError {
                    backend: ApiBackend::MediaFoundation,
                    error: result.to_string(),
                });
            }
            if let Err(error) = unsafe { MFStartup(MF_API_VERSION, MFSTARTUP_NOSOCKET) } {
                if owns_com {
    ...
    fn query_activate_pointers() -> Result<Vec<IMFActivate>, NokhwaError> {
        ...
        let mut count: u32 = 0;
        let mut pointers = std::ptr::null_mut();

        if let Err(why) = unsafe { MFEnumDeviceSources(&attributes, &mut pointers, &mut count) } {
        ...
        // Transfer the owned interface references before freeing the COM array.
        // Cloning these pointers leaked the original reference on every scan.
        let device_list = if count == 0 {
            Vec::new()
        } else {
            unsafe { from_raw_parts_mut(pointers, count as usize) }
                .iter_mut()
                .filter_map(Option::take)
                .collect()
        };
        unsafe { CoTaskMemFree(Some(pointers.cast())) };

        Ok(device_list)
    }

    fn allocated_string(attributes: &IMFActivate, key: &GUID) -> Result<String, NokhwaError> {
        let mut value = PWSTR::null();
        let mut length = 0;
        let result = unsafe { attributes.GetAllocatedString(key, &mut value, &mut length) }
            .map_err(|error| error.to_string())
            .and_then(|()| {
                if value.is_null() {
                    Err("Camera attribute returned a null string".to_owned())
                } else {
                    unsafe { value.to_string() }.map_err(|error| error.to_string())
                }
            });
        unsafe { CoTaskMemFree(Some(value.0.cast())) };
        result.map_err(|error| NokhwaError::GetPropertyError {
            property: format!("{key:?}"),
            error,
        })
    }
    ...
    fn activate_to_descriptors(
        ...
        let name = allocated_string(imf_activate, &MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME)?;
        let symlink = allocated_string(
            imf_activate,
    ...
    pub struct MediaFoundationDevice {
        ...
        // Keep this last so COM interfaces are dropped before their runtime.
    ...
    mod tests {
        ...
        use windows::Win32::System::Com::{
            CoGetApartmentType, APTTYPE, APTTYPEQUALIFIER, APTTYPEQUALIFIER_NONE, APTTYPE_MTA,
        ...
        #[test]
        fn scans_and_capture_initialize_independent_threads() {
            std::thread::spawn(|| {
                let scan = MediaFoundationRuntime::new().unwrap();
                std::thread::spawn(|| {
                    let capture = MediaFoundationRuntime::new().unwrap();
                    assert!(capture.owns_com);
                    ...
                    let nested = MediaFoundationRuntime::new().unwrap();
                    drop(nested);
                    assert!(apartment().is_ok());
                    drop(capture);
                    // The other thread's MTA may remain implicitly available;
                    // a subsequent explicit initialization must still succeed.
                    drop(MediaFoundationRuntime::new().unwrap());
                })
                .join()
                .unwrap();
                assert!(apartment().is_ok());
        ...
        #[test]
        fn borrowed_sta_is_not_uninitialized_by_camera_cleanup() {
            std::thread::spawn(|| {
                unsafe {
                    CoInitializeEx(None, COINIT_APARTMENTTHREADED).ok().unwrap();
                }
                let original = apartment().unwrap();
                let runtime = MediaFoundationRuntime::new().unwrap();
                assert!(!runtime.owns_com);
                drop(runtime);
                assert_eq!(apartment().unwrap(), original);
                unsafe {
                    CoUninitialize();
                }
            })
